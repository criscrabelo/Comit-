...
**Competência:** todas  
**Executado em:** 2026-08-06T11:23:22.063Z  
...
| Métrica | Valor |
| --- | --- |
| Identificador da execução | `c1edd84a-0d09-498a-af65-a364f9c5c043` |
| Quadro | Notificações a Clientes — board `5630368737` |
| Início | 2026-08-06T11:23:22.090Z |
| Conclusão | 2026-08-06T11:23:44.532Z |
| Duração | 22.44 s |
...
| Status | erro (parcial) |
| Contabilidade fecha | NÃO |

**Estado da tabela após a execução:**

| Item | Valor |
| --- | --- |
| Registros com `fonte = 'monday'` | 0 |
| Vivos / marcados ausentes | 0 / 0 |
| `id_origem` distintos | 0 |
| Maior versão | 0 |
...
> **Colunas do mapa não encontradas no quadro:** cpf_cnpj, contrato, unidade, torre, situacao, modelo, acordo, data_solucao, total_dias, saldo_vencido, saldo_atualizado, dias_atraso. Gravadas como nulas, nunca presumidas.
...
#### MODELOS DE NOTIFICAÇÃO _(não mapeada)_
...
| Métrica | Valor |
| --- | --- |
| Identificador da execução | `6d0df311-c2df-4bfe-abc5-aa6945e1820b` |
| Quadro | Notificações a Clientes — board `5630368737` |
| Início | 2026-08-06T11:23:44.584Z |
| Conclusão | 2026-08-06T11:24:07.625Z |
| Duração | 23.04 s |
| **Quantidade recebida** (lidos da origem) | **1072** |
| **Páginas consultadas** | **6** |
| **Último cursor** | `null` — leitura chegou ao fim |
...
| --- | --- | --- | --- |
| 1 | Ausência de duplicação | ✅ | 0 → 0 registros; 0 `id_origem` distintos |
| 2 | Upsert idempotente | ✅ | 2ª execução: 0 incluídos, 0 atualizados, 0 inalterados sobre 1072 lidos |
| 3 | Preservação de `fonte` e `id_origem` | ✅ | 0 sem fonte correta, 0 sem `id_origem` |
| 4 | Registros inalterados não geram versões indevidas | ✅ | histórico 0 → 0; versão máxima 0 → 0 |
| 5 | Registros alterados são atualizados corretamente | ❌ | nenhum registro para testar |
| 6 | Falha posterior não apaga o último dado válido | ✅ | após falha: 0 vivos (era 0); `ultima_carga_valida_em` não avançou; status `erro` |
| 7 | Mutation e subscription continuam bloqueadas | ✅ | 4 tentativas de escrita recusadas; consulta de leitura aceita |

## Amostra anonimizada

_Nenhum registro para amostrar._

## Inconsistências encontradas

| Tipo | Gravidade | Ocorrências | Descrição |
| --- | --- | --- | --- |
| falha_importacao | alta | 3 | Falha ao sincronizar (JUR) NOTIFICAÇÕES CLIENTES: insert or update on table "notificacoes" violates foreign key constraint "notificacoes_com |

---

**1 prova(s) falharam.** A homologação NÃO deve ser aprovada nesta condição.
